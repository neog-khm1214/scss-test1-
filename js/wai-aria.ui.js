    ...
    tab.addEventListener("click", (e) => {
      const parent =
        tab.parentNode.tagName === "LI"
          ...
          : tab.parentNode;
      const panelWrap = document.querySelector(
...
var waiAriaBtnPress = function () {
  const pressButtons = document.querySelectorAll(
    '[role="button"][aria-pressed], button[aria-pressed]'
  );
...
var waiAriaSlider = function () {
  ...
  sliders.forEach((slider) => {
    const maxVal = slider.getAttribute("aria-valuemax")
      ? slider.getAttribute("aria-valuemax")
      : 100;
    const sliderWrap = slider.parentNode;
    const slideTo = slider.getAttribute("aria-orientation")
      ? slider.getAttribute("aria-orientation")
      : null;

    if (
      window.getComputedStyle(sliderWrap).getPropertyValue("position") !==
      "relative"
    )
  ...
  function sliderMouseEvent(
    ...
    const wrapPos =
      slideTo && slideTo == "vertical"
        ? sliderWrap.getBoundingClientRect().top
        : sliderWrap.getBoundingClientRect().left;
    const mousePos = slideTo && slideTo == "vertical" ? e.pageY : e.pageX;
    const leftPos =
      mousePos - wrapPos < 0
        ? 0
        : mousePos - wrapPos > wrapWidth
        ? wrapWidth
        : mousePos - wrapPos;
  ...
  function sliderKeyEvent(slideTo, slider, maxVal, addition) {
    const nowVal = parseInt(slider.getAttribute("aria-valuenow"))
    ...
    const stylePos =
      maxVal < changePos ? 100 : Math.round((changePos / maxVal) * 100);
    const sliderWidth =
      slideTo && slideTo == "vertical"
        ? slider.offsetHeight
        : slider.offsetWidth;
...
var waiAriaMenuBar = function () {
  ...
  menuBars.forEach((menuBar) => {
    ...
    for (let i = 0; i < hasSubs.length; i++) {
      subPops.push(
        menuBar.querySelector(`#${hasSubs[i].getAttribute("aria-controls")}`)
      );
    }
...
var waiAriaHasPopupMenu = function () {
  ...
  haspops.forEach((haspop, index) => {
    const popup = document.querySelector(
      `#${haspop.getAttribute("aria-controls")}`
    );
    ...
    const hasSubs = Array.from(
      popup.querySelectorAll('[aria-haspopup="true"]')
    );
    const subPops = [];
    for (let i = 0; i < hasSubs.length; i++) {
      subPops.push(
        popup.querySelector(`#${hasSubs[i].getAttribute("aria-controls")}`)
      );
    }
    for (let i = 0; i < subPops.length; i++) {
      menus = menus.filter((menu) => !subPops[i].contains(menu));
    ...
    haspop.addEventListener("click", (e) => {
      ...
      if (popup.style.display == "none") {
        hasSubs.forEach((hasSub) => {
          const subpop = popup.querySelector(
            `#${hasSub.getAttribute("aria-controls")}`
          );
          if (!subpop) return;
    ...
    haspop.addEventListener("click", (e) => {
      ...
      menus.forEach((menu) => {
        if (
          ...
          subpopups.push(
            document.querySelector(`#${menu.getAttribute("aria-controls")}`)
          );
